...
TEST_F(TestSort,test_merge){
    ...
    m_sort.get()->sort_merge(a4,11);
    int b2[] = {1,2,6,12,12,12,23,44,55,66,100};
    for (int i = 0; i < 11; ++i) {
        GTEST_ASSERT_EQ(b2[i],a4.get()[i]);
    }
    ...
    m_sort.get()->sort_merge(a2,7);
    for (int i = 0; i < 7; ++i) {
        GTEST_ASSERT_EQ(14 + i,a2.get()[i]);
    }
}

TEST_F(TestSort,test_quick){
    auto a1 = std::make_shared<int>(6);
    for (int i = 0; i < 6; ++i) {
        a1.get()[i] = i;
    }
    m_sort.get()->sourt_quick(a1,6);
    for (int i = 0; i < 6; ++i) {
        GTEST_ASSERT_EQ(i,a1.get()[i]);
    }
